...
class Nodo:
    def __init__(self, _nodo, estado, vazio, numero_pais, custo=0):
        ...
        self.custo_parcial = custo + self.custo() + numero_pais
        self.total_custo = self.custo_parcial + self.heuristica()
    ...
    def heuristica(self):
        ...
        for i in range(3):
            for j in range(3):
                if i==2 and j == 2:
                    if self.estado[i][j] is not None:
                        counter +=1

                else:
                    if self.estado[i][j] != (i * 3) + (j + 1) :
                        counter +=1
...
class HeuristicaForte:
    ...
    def gera_nodo(self, tar_1,tar_2, des_1,des_2):
        ...
        return Nodo(self.nodo, estado, [des_1,des_2], self.nodo.numero_pais + 1, self.nodo.custo_parcial)
...
HeuristicaForte([[6,2,8], [1,7,None], [ 4,5, 3]]).busca()
